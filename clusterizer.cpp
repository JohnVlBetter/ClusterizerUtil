...
namespace meshopt
{
	...
	static void finishMeshlet(meshopt_Meshlet& meshlet, unsigned int* meshlet_triangles)
	{
	...
	static bool appendMeshlet(meshopt_Meshlet& meshlet, unsigned int a, unsigned int b, unsigned int c, unsigned char* used, meshopt_Meshlet* meshlets,
		unsigned int* meshlet_vertices, unsigned int* meshlet_triangles, size_t meshlet_offset, size_t max_vertices, size_t max_triangles)
	{
	...
	static unsigned int getNeighborTriangle(const meshopt_Meshlet& meshlet, const Cone* meshlet_cone, unsigned int* meshlet_vertices,
		const unsigned int* indices, const TriangleAdjacency2& adjacency, const Cone* triangles, const unsigned int* live_triangles,
		const unsigned char* used, float meshlet_expected_radius, float cone_weight, unsigned int* out_extra)
	{
	...
	static unsigned int getNeighborTriangleByTriangle(const unsigned int* triIndices, unsigned int* meshlet_vertices,
		const unsigned int* indices, const TriangleAdjacency2& adjacency, const Cone* triangles, const unsigned int* live_triangles,
		const unsigned char* used, float meshlet_expected_radius, float cone_weight, size_t face_count)
	{
		unsigned int best_triangle = ~0u;
		unsigned int best_extra = 5;
		float best_score = FLT_MAX;
		...
		//拿当前三角形的cone
		const Cone* meshlet_cone = &(triangles[triIndices[0]]);
		bool isVaild = triIndices[0] >= face_count;

		for (size_t i = 0; i < 3; ++i)
		{
			unsigned int index = triIndices[i];
...
meshopt_Bounds meshopt_computeMeshletBounds(const unsigned int* meshlet_vertices,
	const unsigned int* meshlet_triangles, size_t triangle_count, const float* vertex_positions, size_t vertex_count, size_t vertex_positions_stride)
{
	using namespace meshopt;
	...
	size_t max_triangles, float cone_weight)
{
	...
	for (;;)
	{
		...
		unsigned int best_triangle = getNeighborTriangle(meshlet, &meshlet_cone, meshlet_vertices, indices, adjacency, triangles, live_triangles,
			used, meshlet_expected_radius, cone_weight, &best_extra);
		...
		//获取一个best_triangle的邻接三角形，组成一个四边形一起塞进meshlet
		unsigned int triIndices[3] = { a, b, c };
		//triIndices[0] = a;
		//triIndices[1] = b;
		//triIndices[2] = c;
		unsigned int another_best_triangle = getNeighborTriangleByTriangle(triIndices, meshlet_vertices, indices, adjacency,
			triangles, live_triangles, used, meshlet_expected_radius, cone_weight, face_count);
		//找不到邻接三角形就用他自己
		if (another_best_triangle == ~0u)
		{
			another_best_triangle = best_triangle;
		}
		unsigned int a1 = indices[another_best_triangle * 3 + 0], b1 = indices[another_best_triangle * 3 + 1], c1 = indices[another_best_triangle * 3 + 2];
		assert(a1 < vertex_count&& b1 < vertex_count&& c1 < vertex_count);
		...
		if (appendMeshlet(meshlet, a1, b1, c1, used, meshlets, meshlet_vertices, meshlet_triangles, meshlet_offset, max_vertices, max_triangles))
		{
			meshlet_offset++;
			memset(&meshlet_cone_acc, 0, sizeof(meshlet_cone_acc));
		}
		...
		for (size_t k = 0; k < 3; ++k)
		{
			unsigned int index = indices[another_best_triangle * 3 + k];
		...
		meshlet_cone_acc.px += triangles[another_best_triangle].px;
		meshlet_cone_acc.py += triangles[another_best_triangle].py;
		meshlet_cone_acc.pz += triangles[another_best_triangle].pz;
		meshlet_cone_acc.nx += triangles[another_best_triangle].nx;
		meshlet_cone_acc.ny += triangles[another_best_triangle].ny;
		meshlet_cone_acc.nz += triangles[another_best_triangle].nz;

		emitted_flags[another_best_triangle] = 1;
	}
	...
	for (size_t i = 0; i < meshlet_offset; ++i)
	{
		...
		memcpy(meshlets[i].center, bound.center, 3*sizeof(float));
		meshlets[i].radius = bound.radius;
	...
	auto last = meshlets[meshlet_offset - 1];
	int triC = last.triangle_offset + ((last.triangle_count * 3 + 3) & ~3);
	std::vector<unsigned int> tmp_meshlet_triangles(triC);
	for (int idx = 0; idx < meshlet_offset; ++idx)
	{
		auto curMeshlet = meshlets[idx];
		std::vector<unsigned int> curMeshletVertices(curMeshlet.vertex_count);
		for (int j = 0; j < curMeshlet.vertex_count; ++j)
		{
			curMeshletVertices[j] = meshlet_vertices[curMeshlet.vertex_offset + j];
		}

		auto triangle_count = meshlets[idx].triangle_count;
		for (int j = 0; j < triangle_count; ++j)
		{
			meshlet_triangles[curMeshlet.triangle_offset + j * 3] = curMeshletVertices[meshlet_triangles[curMeshlet.triangle_offset + j * 3]];
			meshlet_triangles[curMeshlet.triangle_offset + j * 3 + 1] = curMeshletVertices[meshlet_triangles[curMeshlet.triangle_offset + j * 3 + 1]];
			meshlet_triangles[curMeshlet.triangle_offset + j * 3 + 2] = curMeshletVertices[meshlet_triangles[curMeshlet.triangle_offset + j * 3 + 2]];
		}
	}
...
size_t meshopt_buildMeshletsScan(meshopt_Meshlet* meshlets, unsigned int* meshlet_vertices, unsigned int* meshlet_triangles, const unsigned int* indices, size_t index_count, size_t vertex_count, size_t max_vertices, size_t max_triangles)
{
	using namespace meshopt;
